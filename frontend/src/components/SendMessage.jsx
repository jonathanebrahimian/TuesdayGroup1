...
export class SendMessage extends React.Component {
  ...
  render() {
    return (
      <>
      <h3>You are sending a message to {this.props.soldierName}</h3>
      <form>
        <label htmlFor="message">Attach Message</label>
        <br/>
        <textarea id="message" placeholder="Message" name="message" rows="5" className="col-10" value={this.state.message} onChange={event => this.setState({ message: event.target.value })}></textarea> 
        <br/>
